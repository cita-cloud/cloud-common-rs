...
pub mod network_service_server {
    ...
    pub const SERVICE_NAME: &str = "network.NetworkService";
...
pub mod network_msg_handler_service_server {
    ...
    #[async_trait]
    pub trait NetworkMsgHandlerService: std::marker::Send + std::marker::Sync + 'static {
    ...
    /// modules which need network must implement this service
    #[derive(Debug)]
    pub struct NetworkMsgHandlerServiceServer<T> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T> NetworkMsgHandlerServiceServer<T> {
        pub fn new(inner: T) -> Self {
            Self::from_arc(Arc::new(inner))
        }
        pub fn from_arc(inner: Arc<T>) -> Self {
    ...
    impl<T> Clone for NetworkMsgHandlerServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "network.NetworkMsgHandlerService";
    impl<T> tonic::server::NamedService for NetworkMsgHandlerServiceServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
