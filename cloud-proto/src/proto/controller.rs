...
pub mod rpc_service_server {
    ...
    pub const SERVICE_NAME: &str = "controller.RPCService";
...
pub mod consensus2_controller_service_server {
    ...
    #[async_trait]
    pub trait Consensus2ControllerService: std::marker::Send + std::marker::Sync + 'static {
        /// Consensus request a Proposal to start consensus
        /// ret: proposal
    ...
    #[derive(Debug)]
    pub struct Consensus2ControllerServiceServer<T> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T> Consensus2ControllerServiceServer<T> {
        pub fn new(inner: T) -> Self {
            Self::from_arc(Arc::new(inner))
        }
        pub fn from_arc(inner: Arc<T>) -> Self {
    ...
    impl<T> Clone for Consensus2ControllerServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "controller.Consensus2ControllerService";
    impl<T> tonic::server::NamedService for Consensus2ControllerServiceServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
